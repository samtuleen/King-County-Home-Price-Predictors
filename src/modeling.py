...
#     print("Rainbow p-value:", rainbow_p_value)
    ...
    ax[1].plot(modelname.predict(), [0 for i in range(len(df))], color = 'maroon')
    fig.tight_layout()
    return plt.show()
...
def z_score(feature, df):
    """
    feaure is a string of the feature whose z-score to obtain 
    df is the dataframe in which the feature can be found
    """
    z_name = 'z_' + feature
...
def heatmap_multi(x_features, df):
    """
    Creates a heatmap of all the x features in a model to show multicollinearity.
...
def cohen_d(group1, group2):
    ...
    print(f"Cohens d Effect Size for Difference in Home Prices of The 2 Groups: {d}")
...
def ttest_vis(group1, group2, test_result):
    """
    Takes 2 samples and a t-test result and returns a visualization of the critical value and t statistic.
    """
    ...
    ax.plot(xs, ys, linewidth=3, color='maroon', alpha=.75)
